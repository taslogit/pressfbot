    ...
    <div className="pt-4 pb-24 relative min-h-[80vh]">
      <div className="relative z-10">
        ...
        <div className="bg-card/70 border border-border rounded-2xl p-5 shadow-xl">
          <div className="text-[10px] uppercase tracking-widest text-muted mb-2">{headline}</div>
          <div className="text-lg font-black text-primary mb-2">{subtitle}</div>
          <div className="text-xs text-muted mb-4">{caption}</div>

          <div className="mb-3 text-[10px] uppercase tracking-widest text-muted">{t('share_template_title')}</div>
          <div className="flex flex-wrap gap-2 mb-4">
            {templates.map((item) => (
              <button
                key={item.id}
                onClick={() => setTemplateId(item.id)}
                className={`px-3 py-1 rounded-full text-[10px] uppercase tracking-widest border transition-all ${item.chipClass} ${
          ...
          <div className="mb-3 text-[10px] uppercase tracking-widest text-muted">{t('share_message_title')}</div>
          <div className="flex flex-wrap gap-2 mb-4">
            {messageVariants.map((text, index) => (
              <button
                key={`${index}-${text.slice(0, 12)}`}
                onClick={() => setMessageId(index)}
                className={`px-3 py-1 rounded-full text-[10px] uppercase tracking-widest border transition-all border-border ${
          ...
          <div ref={previewRef} className={`relative overflow-hidden border rounded-xl p-4 font-mono text-xs ${template.cardClass}`}>
            ...
            <div className="mt-3 pt-3 border-t border-border/60">
              <div className="text-[10px] uppercase tracking-widest text-muted mb-2">{t('share_timeline_title')}</div>
              <div className="space-y-2">
                <div className="flex items-start gap-2">
                  <span className={`mt-1 h-2 w-2 rounded-full ${template.accentClass}`} />
                  <div>
                    <div className="text-[10px] text-muted">{nowStamp}</div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    <div className="text-[10px] text-muted">{t('share_timeline_actor')}</div>
                ...
                <div className="flex items-start gap-2">
                  <span className="mt-1 h-2 w-2 rounded-full bg-white/40" />
                  <div>
                    <div className="text-[10px] text-muted">{t('share_timeline_object')}</div>
            ...
            <div className="mt-3 pt-3 border-t border-border/60 flex items-center justify-between text-[10px] uppercase tracking-widest text-muted">
              <span>PRESS F</span>
          ...
          <button
            onClick={handleShare}
            className="mt-4 w-full px-3 py-2 rounded-lg border border-accent-cyan/40 text-accent-cyan bg-accent-cyan/10 hover:bg-accent-cyan/20 transition-all font-bold tracking-widest text-[10px] uppercase flex items-center justify-center gap-2"
          >
            <Share2 size={14} /> {t('share_post_cta')}
          </button>
          <button
            onClick={handleCopy}
            className="mt-2 w-full px-3 py-2 rounded-lg border border-border text-muted hover:text-primary transition-all font-bold tracking-widest text-[10px] uppercase flex items-center justify-center gap-2"
          >
